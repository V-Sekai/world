...
func recenter() -> bool:
	...
	camera_transform = camera_transform.looking_at(
		camera_transform.origin + forward_dir.normalized(), Vector3.UP, true
	)
...
func _process_on_physical_movement(delta) -> bool:
	...
	if (
		!is_instance_valid(current_characterbody3D)
		or !is_instance_valid(xr_origin_3D)
		or !is_instance_valid(xr_camera_3D)
	):
		return false
	...
	current_characterbody3D.transform.basis = current_characterbody3D.transform.basis.rotated(
		Vector3.UP, angle
	)
	...
	var player_body_location: Vector3 = (
		xr_origin_3D.transform * xr_camera_3D.transform * xr_neck_position_3D.transform.origin
	)
	...
	if is_instance_valid(camera_3d):
		xr_origin_3D.transform.origin.y = (
			(camera_3d.transform.origin.y - current_characterbody3D.transform.origin.y)
			- xr_camera_3D.transform.origin.y
	...
	var location_offset = (
		(player_body_location - current_characterbody3D.global_transform.origin).length()
	)
...
func set_enabled(
	value: bool,
	new_origin,
	reverse_roomscale: bool = false,
	current_camera: Camera3D = null,
	height_adjustment: float = 0.0
) -> bool:
	if (
		value == true
		and (current_characterbody3D == null or !is_instance_valid(current_characterbody3D))
	):
		print(
			"Tried to enable roomscale but characterbody3D still not set or is set to an invalid instance."
		)
		return false
