...
func add_entry(entry: String) -> void:
	...
	for i in range(len(menu_quads)):
		menu_quads[i].transform.origin = (
			transform.origin
			+ Vector3(sin(arc_size * i) * menu_scale, cos(arc_size * i) * menu_scale, 0)
		)
...
func entry_from_position(position: Vector3) -> String:
	...
	var controller_vector = (
		inverse_transform * position - inverse_transform * radial_menu_open_position.origin
	)
	var first_entry_vector = (
		inverse_transform * menu_quads[0].global_transform.origin
		- inverse_transform * radial_menu_open_position.origin
	)
...
#		$Anchor.global_transform = radial_menu_open_position
		if (
			(anchor.global_transform.origin - xr_controller.global_transform.origin).length()
			>= distance_to_follow
		):
			anchor.global_transform.origin = lerp(
				anchor.global_transform.origin, xr_controller.global_transform.origin, delta
			)
