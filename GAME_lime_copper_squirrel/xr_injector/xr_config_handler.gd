...
var ugvr_menu_toggle_combo: Dictionary = {
	"primary_controller": ["primary_click"], "secondary_controller": ["primary_click"]
}
...
func load_game_options_cfg_file(file_path: String) -> bool:
	...
	# Load camera options
	xr_world_scale = game_options_cfg_file.get_value(
		"CAMERA_OPTIONS", "xr_world_scale", xr_world_scale
	)
	camera_offset = game_options_cfg_file.get_value(
		"CAMERA_OPTIONS", "camera_offset", camera_offset
	)
	experimental_passthrough = game_options_cfg_file.get_value(
		"CAMERA_OPTIONS", "experimental_passthrough", experimental_passthrough
	)

	# Load viewport options
	xr_main_viewport_location = game_options_cfg_file.get_value(
		"VIEWPORTS_OPTIONS", "xr_main_viewport_location", xr_main_viewport_location
	)
	xr_secondary_viewport_location = game_options_cfg_file.get_value(
		"VIEWPORTS_OPTIONS", "xr_secondary_viewport_location", xr_secondary_viewport_location
	)
	primary_viewport_size_multiplier = game_options_cfg_file.get_value(
		"VIEWPORTS_OPTIONS", "primary_viewport_size_multiplier", primary_viewport_size_multiplier
	)
	secondary_viewport_size_multiplier = game_options_cfg_file.get_value(
		"VIEWPORTS_OPTIONS",
		"secondary_viewport_size_multiplier",
		secondary_viewport_size_multiplier
	)
	primary_viewport_offset = game_options_cfg_file.get_value(
		"VIEWPORTS_OPTIONS", "primary_viewport_offset", primary_viewport_offset
	)
	secondary_viewport_offset = game_options_cfg_file.get_value(
		"VIEWPORTS_OPTIONS", "secondary_viewport_offset", secondary_viewport_offset
	)

	# Load roomscale options
	use_roomscale = game_options_cfg_file.get_value(
		"ROOMSCALE_OPTIONS", "use_roomscale", use_roomscale
	)
	roomscale_height_adjustment = game_options_cfg_file.get_value(
		"ROOMSCALE_OPTIONS", "roomscale_height_adjustment", roomscale_height_adjustment
	)
	attempt_to_use_camera_to_set_roomscale_height = game_options_cfg_file.get_value(
		"ROOMSCALE_OPTIONS",
		"attempt_to_use_camera_to_set_roomscale_height",
		attempt_to_use_camera_to_set_roomscale_height
	)
	reverse_roomscale_direction = game_options_cfg_file.get_value(
		"ROOMSCALE_OPTIONS", "reverse_roomscale_direction", reverse_roomscale_direction
	)
	use_roomscale_3d_cursor = game_options_cfg_file.get_value(
		"ROOMSCALE_OPTIONS", "use_roomscale_3d_cursor", use_roomscale_3d_cursor
	)
	use_long_range_3d_cursor = game_options_cfg_file.get_value(
		"ROOMSCALE_OPTIONS", "use_long_range_3d_cursor", use_long_range_3d_cursor
	)
	roomscale_3d_cursor_distance_from_camera = game_options_cfg_file.get_value(
		"ROOMSCALE_OPTIONS",
		"roomscale_3d_cursor_distance_from_camera",
		roomscale_3d_cursor_distance_from_camera
	)
	roomscale_long_range_3d_cursor_distance_from_camera = game_options_cfg_file.get_value(
		"ROOMSCALE_OPTIONS",
		"roomscale_long_range_3d_cursor_distance_from_camera",
		roomscale_long_range_3d_cursor_distance_from_camera
	)
	use_arm_swing_jump = game_options_cfg_file.get_value(
		"ROOMSCALE_OPTIONS", "use_arm_swing_jump", use_arm_swing_jump
	)
	use_jog_movement = game_options_cfg_file.get_value(
		"ROOMSCALE_OPTIONS", "use_jog_movement", use_jog_movement
	)
	jog_triggers_sprint = game_options_cfg_file.get_value(
		"ROOMSCALE_OPTIONS", "jog_triggers_sprint", jog_triggers_sprint
	)

	# Load autosave options
	autosave_action_map_duration_in_secs = game_options_cfg_file.get_value(
		"AUTOSAVE_OPTIONS",
		"autosave_action_map_duration_in_secs",
		autosave_action_map_duration_in_secs
	)

	# Load xr injector GUI options
	show_welcome_label = game_options_cfg_file.get_value(
		"XR_INJECTOR_GUI_OPTIONS", "show_welcome_label", show_welcome_label
	)
...
func save_game_options_cfg_file(file_path):
	...
	game_options_cfg_file.set_value(
		"CAMERA_OPTIONS", "experimental_passthrough", experimental_passthrough
	)
...
func create_action_map_cfg_file(file_path):
		...
		if not action in default_action_map_actions:
			...
			for event in game_action_events:
				...
				if not event is InputEventJoypadButton and not event is InputEventJoypadMotion:
					...
					action_map_cfg_file.set_value(
						"GAME_ACTIONS", action, default_gamepad_button_names[event.button_index]
					)
					...
					action_map_cfg_file.set_value(
						"GAME_ACTIONS",
						action,
						[default_joystick_axis_names[event.axis], event.axis_value]
					)
					event_already_set_for_action = true
	...
	action_map_cfg_file.set_value(
		"RADIAL_MENU_OPTIONS", "xr_radial_menu_entries", xr_radial_menu_entries
	)
	action_map_cfg_file.set_value(
		"RADIAL_MENU_OPTIONS", "open_radial_menu_button", open_radial_menu_button
	)
...
func load_action_map_file(file_path: String) -> bool:
			...
			if typeof(value) == TYPE_STRING or typeof(value) == TYPE_STRING_NAME:
				...
				if button_index != -1:
					...
					for original_event in InputMap.action_get_events(action):
						if (
							original_event.is_class("InputEventJoypadButton")
							or original_event.is_class("InputEventJoypadMotion")
						):
							print(
								"Original action map had joypad event assigned, deleting for new event"
							)
				...
				if axis_index != -1:
					...
					if ugvr_menu_2d:
						...
						for index in range(15, 25):
							if (
								new_settings_button.get_item_text(index).contains(axis_name)
								and new_settings_button.get_item_text(index).contains(
									str(axis_value)
								)
							):
	...
	use_xr_radial_menu = action_map_cfg_file.get_value(
		"RADIAL_MENU_OPTIONS", "use_xr_radial_menu", use_xr_radial_menu
	)
	xr_radial_menu_mode = action_map_cfg_file.get_value(
		"RADIAL_MENU_OPTIONS", "xr_radial_menu_mode", xr_radial_menu_mode
	)
	xr_radial_menu_entries = action_map_cfg_file.get_value(
		"RADIAL_MENU_OPTIONS", "xr_radial_menu_entries", xr_radial_menu_entries
	)
	open_radial_menu_button = action_map_cfg_file.get_value(
		"RADIAL_MENU_OPTIONS", "open_radial_menu_button", open_radial_menu_button
	)
...
func save_game_control_map_cfg_file(file_path):
	...
	for key in primary_action_map:
		game_control_map_cfg_file.set_value(
			"PRIMARY_CONTROLLER", key, default_gamepad_button_names[primary_action_map[key]]
		)
	...
	for key in secondary_action_map:
		game_control_map_cfg_file.set_value(
			"SECONDARY_CONTROLLER", key, default_gamepad_button_names[secondary_action_map[key]]
		)

	# Set mouse emulation options

	game_control_map_cfg_file.set_value(
		"MOUSE_EMULATION_OPTIONS", "stick_emulate_mouse_movement", stick_emulate_mouse_movement
	)

	game_control_map_cfg_file.set_value(
		"MOUSE_EMULATION_OPTIONS", "head_emulate_mouse_movement", head_emulate_mouse_movement
	)

	game_control_map_cfg_file.set_value(
		"MOUSE_EMULATION_OPTIONS",
		"primary_controller_emulate_mouse_movement",
		primary_controller_emulate_mouse_movement
	)

	game_control_map_cfg_file.set_value(
		"MOUSE_EMULATION_OPTIONS",
		"secondary_controller_emulate_mouse_movement",
		secondary_controller_emulate_mouse_movement
	)
	...
	game_control_map_cfg_file.set_value(
		"MOUSE_EMULATION_OPTIONS", "emulated_mouse_deadzone", emulated_mouse_deadzone
	)
	...
	game_control_map_cfg_file.set_value(
		"OTHER_CONTROL_OPTIONS", "stick_turn_controller", stick_turn_controller
	)

	game_control_map_cfg_file.set_value("OTHER_CONTROL_OPTIONS", "grip_deadzone", grip_deadzone)

	game_control_map_cfg_file.set_value(
		"OTHER_CONTROL_OPTIONS", "primary_controller_selection", primary_controller_selection
	)

	game_control_map_cfg_file.set_value(
		"OTHER_CONTROL_OPTIONS", "ugvr_menu_toggle_combo", ugvr_menu_toggle_combo
	)

	game_control_map_cfg_file.set_value(
		"OTHER_CONTROL_OPTIONS", "pointer_gesture_toggle_button", pointer_gesture_toggle_button
	)

	game_control_map_cfg_file.set_value(
		"OTHER_CONTROL_OPTIONS", "gesture_load_action_map_button", gesture_load_action_map_button
	)

	game_control_map_cfg_file.set_value(
		"OTHER_CONTROL_OPTIONS", "gesture_set_user_height_button", gesture_set_user_height_button
	)

	game_control_map_cfg_file.set_value(
		"OTHER_CONTROL_OPTIONS", "dpad_activation_button", dpad_activation_button
	)
	...
	game_control_map_cfg_file.set_value(
		"OTHER_CONTROL_OPTIONS", "use_physical_gamepad_only", use_physical_gamepad_only
	)
...
func load_game_control_map_cfg_file(file_path: String) -> bool:
	...
	for key in primary_action_map.keys():
		if game_control_map_cfg_file.has_section_key("PRIMARY_CONTROLLER", key):
			...
			if button_index != -1:
				...
				printerr(
					"Primary controller button mapping error, assigned key not recognized: ",
					button_name
				)
	...
	for key in secondary_action_map.keys():
		if game_control_map_cfg_file.has_section_key("SECONDARY_CONTROLLER", key):
			...
			if button_index != -1:
				...
				printerr(
					"Secondary controller button mapping error, assigned key not recognized: ",
					button_name
				)

	# Load mouse emulation options
	stick_emulate_mouse_movement = game_control_map_cfg_file.get_value(
		"MOUSE_EMULATION_OPTIONS", "stick_emulate_mouse_movement", stick_emulate_mouse_movement
	)

	head_emulate_mouse_movement = game_control_map_cfg_file.get_value(
		"MOUSE_EMULATION_OPTIONS", "head_emulate_mouse_movement", head_emulate_mouse_movement
	)

	primary_controller_emulate_mouse_movement = game_control_map_cfg_file.get_value(
		"MOUSE_EMULATION_OPTIONS",
		"primary_controller_emulate_mouse_movement",
		primary_controller_emulate_mouse_movement
	)

	secondary_controller_emulate_mouse_movement = game_control_map_cfg_file.get_value(
		"MOUSE_EMULATION_OPTIONS",
		"secondary_controller_emulate_mouse_movement",
		secondary_controller_emulate_mouse_movement
	)

	emulated_mouse_sensitivity_multiplier = game_control_map_cfg_file.get_value(
		"MOUSE_EMULATION_OPTIONS",
		"emulated_mouse_sensitivity_multiplier",
		emulated_mouse_sensitivity_multiplier
	)

	emulated_mouse_deadzone = game_control_map_cfg_file.get_value(
		"MOUSE_EMULATION_OPTIONS", "emulated_mouse_deadzone", emulated_mouse_deadzone
	)
	...
	if game_control_map_cfg_file.has_section_key("OTHER_CONTROL_OPTIONS", "turning_speed"):
		turning_speed = game_control_map_cfg_file.get_value(
			"OTHER_CONTROL_OPTIONS", "turning_speed"
		)

	if game_control_map_cfg_file.has_section_key("OTHER_CONTROL_OPTIONS", "turning_degrees"):
		turning_degrees = game_control_map_cfg_file.get_value(
			"OTHER_CONTROL_OPTIONS", "turning_degrees"
		)

	if game_control_map_cfg_file.has_section_key("OTHER_CONTROL_OPTIONS", "stick_turn_controller"):
		stick_turn_controller = game_control_map_cfg_file.get_value(
			"OTHER_CONTROL_OPTIONS", "stick_turn_controller"
		)

	if game_control_map_cfg_file.has_section_key("OTHER_CONTROL_OPTIONS", "grip_deadzone"):
		grip_deadzone = game_control_map_cfg_file.get_value(
			"OTHER_CONTROL_OPTIONS", "grip_deadzone"
		)

	if game_control_map_cfg_file.has_section_key(
		"OTHER_CONTROL_OPTIONS", "primary_controller_selection"
	):
		primary_controller_selection = game_control_map_cfg_file.get_value(
			"OTHER_CONTROL_OPTIONS", "primary_controller_selection"
		)

	if game_control_map_cfg_file.has_section_key("OTHER_CONTROL_OPTIONS", "ugvr_menu_toggle_combo"):
		ugvr_menu_toggle_combo = game_control_map_cfg_file.get_value(
			"OTHER_CONTROL_OPTIONS", "ugvr_menu_toggle_combo"
		)

	if game_control_map_cfg_file.has_section_key(
		"OTHER_CONTROL_OPTIONS", "pointer_gesture_toggle_button"
	):
		pointer_gesture_toggle_button = game_control_map_cfg_file.get_value(
			"OTHER_CONTROL_OPTIONS", "pointer_gesture_toggle_button"
		)

	if game_control_map_cfg_file.has_section_key(
		"OTHER_CONTROL_OPTIONS", "gesture_load_action_map_button"
	):
		gesture_load_action_map_button = game_control_map_cfg_file.get_value(
			"OTHER_CONTROL_OPTIONS", "gesture_load_action_map_button"
		)

	if game_control_map_cfg_file.has_section_key(
		"OTHER_CONTROL_OPTIONS", "gesture_set_user_height_button"
	):
		gesture_set_user_height_button = game_control_map_cfg_file.get_value(
			"OTHER_CONTROL_OPTIONS", "gesture_set_user_height_button"
		)

	if game_control_map_cfg_file.has_section_key("OTHER_CONTROL_OPTIONS", "dpad_activation_button"):
		dpad_activation_button = game_control_map_cfg_file.get_value(
			"OTHER_CONTROL_OPTIONS", "dpad_activation_button"
		)
	...
	if game_control_map_cfg_file.has_section_key("OTHER_CONTROL_OPTIONS", "select_button"):
		select_button = game_control_map_cfg_file.get_value(
			"OTHER_CONTROL_OPTIONS", "select_button"
		)

	if game_control_map_cfg_file.has_section_key(
		"OTHER_CONTROL_OPTIONS", "use_physical_gamepad_only"
	):
		use_physical_gamepad_only = game_control_map_cfg_file.get_value(
			"OTHER_CONTROL_OPTIONS", "use_physical_gamepad_only"
		)
...
func set_ugvr_gui_menu_2d(new_ugvr_menu_2d):
	...
	option_template_label = (
		ugvr_menu_2d
		. get_node(
			"MarginContainer/TabContainer/VR Action Map Options/ScrollContainer/MarginContainer/HSplitContainer/VBoxContainer/OptionLabelPlaceholder"
		)
	)
	action_options = (
		ugvr_menu_2d
		. get_node(
			"MarginContainer/TabContainer/VR Action Map Options/ScrollContainer/MarginContainer/HSplitContainer/VBoxContainer"
		)
	)
	action_settings = (
		ugvr_menu_2d
		. get_node(
			"MarginContainer/TabContainer/VR Action Map Options/ScrollContainer/MarginContainer/HSplitContainer/VBoxContainer2"
		)
	)
	gamepad_button_template = (
		ugvr_menu_2d
		. get_node(
			"MarginContainer/TabContainer/VR Action Map Options/ScrollContainer/MarginContainer/HSplitContainer/VBoxContainer2/GamePadOptionButton"
		)
	)
	xr_face_button_template = (
		ugvr_menu_2d
		. get_node(
			"MarginContainer/TabContainer/VR Action Map Options/ScrollContainer/MarginContainer/HSplitContainer/VBoxContainer2/XRFaceOptionButton"
		)
	)
	checkbox_template = (
		ugvr_menu_2d
		. get_node(
			"MarginContainer/TabContainer/VR Action Map Options/ScrollContainer/MarginContainer/HSplitContainer/VBoxContainer2/DefaultCheckBox"
		)
	)
	section_header_template = (
		ugvr_menu_2d
		. get_node(
			"MarginContainer/TabContainer/VR Action Map Options/ScrollContainer/MarginContainer/HSplitContainer/VBoxContainer/SectionHeaderTemplate"
		)
	)
