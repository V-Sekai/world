...
## Set edited object.
func set_object(object: Object, only_script_vars := true) -> void:
	if is_same(_object, object):
		return

	_object = object
	_only_script_vars = only_script_vars
...
func is_valid_property(property: Dictionary) -> bool:
	var sv := PROPERTY_USAGE_SCRIPT_VARIABLE if _only_script_vars else 0
	if property["hint"] == PROPERTY_HINT_ENUM:
		return property["usage"] == sv + PROPERTY_USAGE_DEFAULT + PROPERTY_USAGE_CLASS_IS_ENUM

	return property["usage"] == sv + PROPERTY_USAGE_DEFAULT
