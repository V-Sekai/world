...
def Variant_GetValue(valobj: lldb.SBValue):
    ...
    if type == VariantType.NIL.value:
        ...
        nodePath: lldb.SBValue = target.CreateValueFromAddress(
        ...
        objData: lldb.SBValue = target.CreateValueFromAddress(
        ...
        dictionary: lldb.SBValue = target.CreateValueFromAddress(
            "dictionary", mem_addr, dictionaryType
        )
        return dictionary
    elif type == VariantType.ARRAY.value:
        arrayType: lldb.SBType = target.FindFirstType("Array")
        array: lldb.SBValue = target.CreateValueFromAddress(
            "array", mem_addr, arrayType
        )
        return array
    elif type == VariantType.PACKED_BYTE_ARRAY.value:
        packedByteArrayType: lldb.SBType = target.FindFirstType(
            "Variant::PackedArrayRef<unsigned char>"
        )
        packedByteArray: lldb.SBValue = target.CreateValueFromAddress(
            "packedByteArrayref", packed_array_addr, packedByteArrayType
        )
        return packedByteArray.CreateValueFromData(
            "packedByteArray",
            packedByteArray.GetChildMemberWithName("array").GetData(),
            target.FindFirstType("PackedByteArray"),
        )
    elif type == VariantType.PACKED_INT32_ARRAY.value:
        packedInt64ArrayType: lldb.SBType = target.FindFirstType(
            "Variant::PackedArrayRef<int>"
        )
        packedInt32Array: lldb.SBValue = target.CreateValueFromAddress(
            "packedInt32Arrayref", packed_array_addr, packedInt64ArrayType
        )
        return packedInt32Array.CreateValueFromData(
            "packedInt32Array",
            packedInt32Array.GetChildMemberWithName("array").GetData(),
            target.FindFirstType("PackedInt32Array"),
        )
    elif type == VariantType.PACKED_INT64_ARRAY.value:
        packedInt64ArrayType: lldb.SBType = target.FindFirstType(
            "Variant::PackedArrayRef<long long>"
        )
        packedInt64Array: lldb.SBValue = target.CreateValueFromAddress(
            "packedInt64Array", packed_array_addr, packedInt64ArrayType
        )
        return packedInt64Array.CreateValueFromData(
            "packedInt64Array",
            packedInt64Array.GetChildMemberWithName("array").GetData(),
            target.FindFirstType("PackedInt64Array"),
        )
    elif type == VariantType.PACKED_FLOAT32_ARRAY.value:
        packedFloat32ArrayType: lldb.SBType = target.FindFirstType(
            "Variant::PackedArrayRef<float>"
        )
        packedFloat32Array: lldb.SBValue = target.CreateValueFromAddress(
            "packedFloat32Array", packed_array_addr, packedFloat32ArrayType
        )
        return packedFloat32Array.CreateValueFromData(
            "packedFloat32Array",
            packedFloat32Array.GetChildMemberWithName("array").GetData(),
            target.FindFirstType("PackedFloat32Array"),
        )
    elif type == VariantType.PACKED_FLOAT64_ARRAY.value:
        packedFloat64ArrayType: lldb.SBType = target.FindFirstType(
            "Variant::PackedArrayRef<double>"
        )
        packedFloat64Array: lldb.SBValue = target.CreateValueFromAddress(
            "packedFloat64Array", packed_array_addr, packedFloat64ArrayType
        )
        return packedFloat64Array.CreateValueFromData(
            "packedFloat64Array",
            packedFloat64Array.GetChildMemberWithName("array").GetData(),
            target.FindFirstType("PackedFloat64Array"),
        )
    elif type == VariantType.PACKED_STRING_ARRAY.value:
        packedStringArrayType: lldb.SBType = target.FindFirstType(
            "Variant::PackedArrayRef<String>"
        )
        packedStringArray: lldb.SBValue = target.CreateValueFromAddress(
            "packedStringArray", packed_array_addr, packedStringArrayType
        )
        return packedStringArray.CreateValueFromData(
            "packedStringArray",
            packedStringArray.GetChildMemberWithName("array").GetData(),
            target.FindFirstType("PackedStringArray"),
        )
    elif type == VariantType.PACKED_COLOR_ARRAY.value:
        packedColorArrayType: lldb.SBType = target.FindFirstType(
            "Variant::PackedArrayRef<Color>"
        )
        packedColorArray: lldb.SBValue = target.CreateValueFromAddress(
            "packedColorArray", packed_array_addr, packedColorArrayType
        )
        return packedColorArray.CreateValueFromData(
            "packedColorArray",
            packedColorArray.GetChildMemberWithName("array").GetData(),
            target.FindFirstType("PackedColorArray"),
        )
    else:
        return None
...
def Variant_SummaryProvider(valobj: lldb.SBValue, internal_dict):
    ...
    if valobj.IsSynthetic():
        return Variant_SyntheticProvider(
            valobj.GetNonSyntheticValue(), internal_dict
        ).get_summary()
    else:
...
def Color_SummaryProvider(valobj, internal_dict):
    return "{{r: {0}, g: {1}, b: {2}, a: {3}}}".format(
        valobj.GetChildMemberWithName("r").GetValueAsSigned(),
        valobj.GetChildMemberWithName("g").GetValueAsSigned(),
        valobj.GetChildMemberWithName("b").GetValueAsSigned(),
        valobj.GetChildMemberWithName("a").GetValueAsSigned(),
    )
...
def Transform2D_SummaryProvider(valobj, internal_dict):
    ...
    return "{{origin= {{{0}}}, x= {{{1}}}, y= {{{2}}}}}".format(
        valobj.GetChildMemberWithName("origin").GetSummary(),
        valobj.GetChildMemberWithName("x").GetSummary(),
        valobj.GetChildMemberWithName("y").GetSummary(),
    )
...
def Array_SummaryProvider(valobj, internal_dict):
    return Array_SyntheticProvider(
        valobj if not valobj.IsSynthetic() else valobj.GetNonSyntheticValue(),
        internal_dict,
    ).get_summary()
...
class Array_SyntheticProvider:
    ...
    def num_children(self):
        try:
            ...
            if _p.GetValueAsUnsigned() == 0:
                return 0
            return getCowDataSize(
                _p.GetChildMemberWithName("array").GetChildMemberWithName("_cowdata")
            )
        except:
            return 0
    ...
    def get_child_at_index(self, index):
        ...
        _ptr: lldb.SBValue = array.GetChildMemberWithName(
            "_cowdata"
        ...
        try:
            ...
            return _ptr.CreateChildAtOffset(
                "[" + str(index) + "]", index * elementSize, type
            )
        except:
            return _ptr.CreateChildAtOffset()
...
def getCowDataSize(_cowdata: lldb.SBValue) -> int:
    ...
    try:
        ...
        if not _ptr.IsValid() or _ptr.GetValueAsUnsigned() == 0:
            return 0
        uint32_type: lldb.SBType = _cowdata.GetTarget().GetBasicType(
            lldb.eBasicTypeUnsignedInt
        )
        _ptr_addr = _ptr.GetValueAsUnsigned()
        _uint32_ptr: lldb.SBValue = _ptr.Cast(uint32_type.GetPointerType())
        size_val: lldb.SBValue = _uint32_ptr.CreateValueFromAddress(
            "size", _ptr_addr - 4, uint32_type
        )
...
def HashMapElement_SummaryProvider(valobj: lldb.SBValue, internal_dict):
    ...
    return "key = {0}, value = {1}".format(
        key_summary if key_summary else get_numeric_string(key),
        value_summary if value_summary else get_numeric_string(value),
    )
...
class HashMap_SyntheticProvider:
    ...
    def get_child_at_index(self, index):
        ...
        try:
            ...
            return element.CreateValueFromData(
...
class Dictionary_SyntheticProvider:
    ...
    def num_children(self):
        ...
        return (
            ...
            .GetChildMemberWithName("num_elements")
            .GetValueAsUnsigned()
        )
    ...
    def get_child_at_index(self, index):
        ...
        try:
            ...
            return HashMap_SyntheticProvider(variant_map, None).get_child_at_index(
                index
            )
        except:
            return None
...
def Vector_SummaryProvider(valobj: lldb.SBValue, internal_dict):
    if valobj.IsSynthetic():
        return Vector_SyntheticProvider(
            valobj.GetNonSyntheticValue(), internal_dict
        ).get_summary()
    else:
...
class Vector_SyntheticProvider:
    ...
    def get_child_at_index(self, index):
        ...
        if self.valobj.IsValid() == False:
            return None
        _ptr: lldb.SBValue = self.valobj.GetChildMemberWithName(
            "_cowdata"
        ).GetChildMemberWithName("_ptr")
        if _ptr.GetValueAsUnsigned() == 0:
            return None
