...
static func plan_to_graph_grammar(todo_list: Array, state: Dictionary) -> RefCounted:
	...
	for i in range(todo_list.size()):
		...
		for tile in possible_tiles:
			...
			var rule = ProductionRule.new("ex:rule" + str(i), "gg:Rule", node, [{"node": tile, "edge": "next"}])
			production_rules.append(rule)

	var graph_grammar = new("ex:myGraphGrammar", "gg:GraphGrammar", node_labels, terminal_node_labels, edge_labels, final_edge_labels, production_rules, initial_nonterminal_symbol)
