...
func _on_control(_user: Node3D, _global_reference: Transform3D, _input: Vector3, rot: Vector3):
	...
	_control_force.x = (
		minf(absf(_control_force.x), absf(_target_control_force.x)) * signf(_target_control_force.x)
	)
	_control_force.y = (
		minf(absf(_control_force.y), absf(_target_control_force.y)) * signf(_target_control_force.y)
	)
	_control_force.z = (
		minf(absf(_control_force.z), absf(_target_control_force.z)) * signf(_target_control_force.z)
	)
	...
	av += (
		calc_angular_velocity(global_basis, Basis.looking_at(global_basis.z, Vector3.UP, true))
		* influence
	)
	av *= total_mass * 0.75
